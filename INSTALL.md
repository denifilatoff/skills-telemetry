...
| `ingest` | The hook path: read an agent Stop payload on stdin, detect skill use (the `[skill-called]` marker, and on Codex the `SKILL.md` reads in the session rollout), queue the events, and flush opportunistically. Always exits 0 so it never fails an agent turn. |
