...
events to a local spool, and forwards them to the collector over OTLP/HTTPS. It runs
...
2. normalizes the detection into one OpenTelemetry log record per skill run (see
   [Data](../README.md#data));
3. writes each record to the on-disk spool;
4. opportunistically flushes the spool to the collector over OTLP/HTTPS.
...
The CLI never blocks an agent turn on the network. `ingest` writes events to the spool
and returns; delivery happens opportunistically.

- **Spool.** One JSON file per buffered event in a per-machine spool directory. A failed
...
- **Flush lock.** `flush` takes a non-blocking advisory lock (`.flush.lock`) on the
  spool, so two concurrent runs never send the same event twice. A run that finds the
...
skips certificate verification; a TLS failure keeps the event in the spool rather than
...
| **Cache** (disposable) | `<UserCacheDir>/qubership-skills-telemetry/` | the binary, `spool/` (one JSON file per event, plus `.lastflush` and `.flush.lock`), `offsets/` (per-session transcript offsets) |
