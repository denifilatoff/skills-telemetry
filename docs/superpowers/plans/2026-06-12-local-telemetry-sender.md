...
| `sender/adapter.go` | `Adapter` interface, `Dispatch(agent, stdin)`, Codex adapter (breadcrumb parse) |
...
The current `detect_skill_call.py` is removed in Task 9 (its breadcrumb logic is ported into the Codex adapter).
...
## Task 3: Codex adapter (breadcrumb parsing)

Ports the breadcrumb logic from `detect_skill_call.py`. The Codex `Stop` payload carries the assistant text in `last_assistant_message`; breadcrumbs have the shape `[skill-called] skill=<name> source=<src>`. One payload may contain several breadcrumbs → several events.
...
git commit -m "feat(sender): add Codex breadcrumb adapter"
```
